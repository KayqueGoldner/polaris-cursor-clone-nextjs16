  ...
  async ({ event, step }) => {
    ...
    const repo = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || "Export from Polaris",
        private: visibility === "private",
        auto_init: true,
      });
    });

    // wait for the repo to be initialized (auto_init is async on GitHub's side)
    ...
    if (fileEntries.length === 0) {
      return new NonRetriableError("No files to export");
    }
    ...
    if (treeItems.length === 0) {
      return new NonRetriableError("Failed to create any file blobs");
    }
